@extends('adminlte::page')

@section('title', 'ステータス一括更新')

@section('content_header')
    <h1>ステータス一括更新</h1>
@stop

@section('content')
<div class="row">
  <div class="col-md-12">
    <div class="card">
      <div class="card-body">
        <form method="GET" action="">
          @csrf
          <div class="row">
            <div class="col-2">
              <input type="text" class="form-control" placeholder="漢字" name="name" value="{{ $request->name }}">
            </div>
            <div class="col-2">
              <input type="text" class="form-control" placeholder="カナ" name="kana" value="{{ $request->kana }}">
            </div>
            <div class="col-2">
              <input type="text" class="form-control" placeholder="携帯" name="tel" value="{{ $request->tel }}">
            </div>

            <div class="col-2">
              <input type="tel" class="form-control" placeholder="緊急連絡先1,2" name="emergency" value="{{ $request->emergency }}">
            </div>

            <div class="col-2">
              <button type="submit" class="btn btn-primary">検索</button>
            </div>
            <div class="col-2"></div>
          </div>
          <div class="row" style="padding:10px 0 0 0;">
            <div class="col-2">
              <input type="text" class="form-control" placeholder="生年月日 2001/01/11" name="birthday" value="{{ $request->birthday }}">
            </div>
            <div class="col-2">
              <select class="form-control" name="status_total">
                <option value="">選択してください</option>
                <option value="査定中" @if($request->status_total == '査定中') selected="" @endif>査定中</option>
                <option value="保留" @if($request->status_total == '保留') selected="" @endif>保留</option>
                <option value="書類不備" @if($request->status_total == '書類不備') selected="" @endif>書類不備</option>
                <option value="査定完了（買取不可）" @if($request->status_total == '査定完了（買取不可）') selected="" @endif>査定完了（買取不可）</option>
                <option value="ケリ" @if($request->status_total == 'ケリ') selected="" @endif>ケリ</option>
                <option value="詐欺" @if($request->status_total == '詐欺') selected="" @endif>詐欺</option>
                <option value="査定完了（買取可）" @if($request->status_total == '査定完了（買取可）') selected="" @endif>査定完了（買取可）</option>
                <option value="代金振込済" @if($request->status_total == '代金振込済') selected="" @endif>代金振込済</option>
              </select>
            </div>
            <div class="col-2">
              <select class="form-control" name="status_paid">
                <option value="">選択してください</option>
                <option value="商品到着待" @if($request->status_paid == '商品到着待') selected="" @endif>商品到着待</option>
                <option value="取引完了" @if($request->status_paid == '取引完了') selected="" @endif>取引完了</option>
                <option value="取引完了遅延" @if($request->status_paid == '取引完了遅延') selected="" @endif>取引完了遅延</option>
                <option value="商品到着済" @if($request->status_paid == '商品到着済') selected="" @endif>商品到着済</option>
              </select>
            </div>

            <div class="col-2">
              <input type="text" class="form-control" placeholder="ユーザー向けメモ" name="for_user" value="{{ $request->for_user }}">
            </div>

            <div class="col-2"></div>
            <div class="col-2"></div>


          </div>
          <input type="hidden" name="concon" value="1">
        </form>
      </div>
    </div>
  </div>
  <div class="col-md-12">
    <div class="card">
      <div class="card-body">
        <form method="POST" action="{{ route('admin.bulkstatus.index') }}" enctype="multipart/form-data">
          @csrf
          <div class="row">
            <div class="col-12">
              <h6>※ステータスを選択してCSVファイルをアップロードしてください。</h6>
              @if($csv_error)
              <p style="color:red;">{{ $csv_error }}</p>
              @endif
            </div>
          </div>
          <div class="row" style="padding:10px 0 0 0;">
            <div class="col-2">
              <select class="form-control" name="status_paid_csv">
                <option value="">ステータス選択</option>
                <option value="商品到着待" @if($request->status_paid_csv == '商品到着待') selected="" @endif>商品到着待</option>
                <option value="取引完了" @if($request->status_paid_csv == '取引完了') selected="" @endif>取引完了</option>
                <option value="取引完了遅延" @if($request->status_paid_csv == '取引完了遅延') selected="" @endif>取引完了遅延</option>
                <option value="商品到着済" @if($request->status_paid_csv == '商品到着済') selected="" @endif>商品到着済</option>
              </select>
            </div>
            <div class="col-2"><input type="text" class="form-control" placeholder="ユーザー向けメモ" name="for_user_csv" id="" value="{{ $request->for_user_csv }}"></div>
            <div class="col-3">
              {{-- <label for="csv_file">CSVファイルを選択してください:</label> --}}
              <input type="file" name="csv_file" id="csv_file">
            </div>

            <div class="col-2">
              <input type="hidden" name="concon" value="2">
              <button type="submit" class="btn btn-primary">アップロード</button>
            </div>

            <div class="col-1"></div>
            {{-- <div class="col-2"></div> --}}
            <div class="col-2"></div>


          </div>
          <input type="hidden" name="concon" value="2">
        </form>
      </div>
    </div>
  </div>





</div>



<div class="row">
  <div class="col-md-12">

    <div class="card">
      <div class="card-header">
        <h3 class="card-title" id="countMessage">
          @if($itemCount)表示されている件数：　  <span id="rowCount">{{ $itemCount }} 件</span>@endif
        </h3>
      </div>
      <!-- /.card-header -->
      <div class="card-body p-0">
        @if($items)

        <form method="POST" action="{{ route('admin.bulkstatus.confirm') }}" id="bulk_form">
          @csrf
        <table class="table table-sm">
          <thead>
            <tr>
              <th width="100"><input type="checkbox" id="all_chk" name="all_chk" value="">全選択</th>
              <th>申込者</th>
              <th>メール</th>
              <th>商品名</th>
              <th>希望金額</th>
              <th>ステータス</th>
              <th>振込後</th>
              <th>査定金額</th>
              <th>承認</th>
              <th>登録日時</th>
              <th>代金振込日時</th>
              <th>ユーザー向けメモ</th>
            </tr>
          </thead>
          <tbody id="tableBody">
            @foreach($items as $item)
            @if($item->user->userData->is_black == 'on')
            <tr class="black filterable">
            @elseif($item->status_total == '査定中')
            <tr class="pink filterable">
            @elseif($item->status_total == '査定完了（買取可）')
            <tr class="blue filterable">
            @else
            <tr class="filterable">
            @endif
              <td>
                <input type="checkbox" name="bulk_id[]" class="bulk_id" value="{{ $item->id }}"> {{ $item->id }}
              </td>
              <td nowrap>{{ $item->user->name }}</td>
              <td>{{ $item->user->email }}</td>
              <td>
                  {{ $item->item_name }}
              </td>
              <td>{{ $item->price }}</td>
              <td>{{ $item->status_total }}</td>
              <td>{{ $item->status_paid }}</td>
              <td>
                @if($item->judge_price)
                {{ $item->judge_price }}円
                @endif
              </td>
              <td>
              @if($item->user_agree)
                <img src="{{ asset('/') }}img/akacheck.png" width="20">
              @endif
              </td>
              <td>{{ $item->created_at }}</td>
              <td class="memo-cell">{{ $item->memo }}</td>
              <td>
                <div style="width:200px;height:70px;overflow:auto;border:1px solid #ccc;font-size:12px;">
                  {{ $item->for_user }}
                </div>
              </td>

            </tr>
            @endforeach
          </tbody>
        </table>
        <div style="text-align:center;margin:20px auto;">
          <button type="submit" id="confirm_btn" class="btn btn-primary">確　認</button>
        </div>
        </form>
        @endif

      </div>
      <!-- /.card-body -->
    </div>
    <!-- /.card -->
  </div>
</div>
@stop

@section('css')
<style type="text/css">
  .black {
    background-color: #ccc;
  }
  .pink {
    background-color: #ffc8c8;
  }
  .blue {
    background-color: #e0efff;
  }
  .table th,td{
    font-size:14px !important;
  }
  .btn-sm{
    font-size:12px;
    min-width:40px;
    text-align:center;
    display: block !important;
  }
</style>
@stop

@section('js')
<script>
  //checkbox 全選択
  $(document).on('click', '#all_chk', function() {
      $("input[name='bulk_id[]']").prop('checked', this.checked);
  });  
  $(document).on('click', '#confirm_btn', function() {
    // チェックボックスが少なくとも1つ選択されているかを確認
    if($(".bulk_id:checked").length === 0) {
        alert("少なくとも1つのチェックボックスを選択してください");
        return false; // フォームの送信をキャンセル
    }else{
      return true;
    }
  
  });  
  </script>

@stop