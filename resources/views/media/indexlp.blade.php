...
<title>メディア集計【情報館】【{{ config('app.name') }}】</title>
...
<div class="content-wrapper ">
    <div class="content-header">
      <div class="container-fluid">
        <h1>メディア集計【情報館】【{{ config('app.name') }}】</h1>
      </div>
    </div>
  
<div class="content">
<div class="container-fluid">
  <p style="font-size:16px;padding:10px;">/lpからの流入を計測しております。</p>
  <div class="row">
    <div class="col-4">
      <div class="card">
          <div class="card-header">{{ __('集計する期間を入力してください。') }}</div>
          <div class="card-body">
            <form method="GET" action="{{route('media.indexlp')}}" class="form-inline my-2 my-lg-0">
            <div class="row">
  ...
  <div class="row">
    <div class="col-md-4">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">※LINE友達登録した時点での集計</h3>
    ...
    <div class="col-md-6">
      ...
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">※LINE友達登録後、買取申込した集計</h3>
          <div class="card-tools">
        ...
        <div class="card-body p-0">
          <table class="table table-striped total_table text-center">
            <thead>
              <tr>
                <th class="bgtb">月日</th>
                <th class="bgtb" colspan="2">LP<br>申込/送金</th>
                <th class="bgtb" colspan="2">LPMT<br>申込/送金</th>
                <th class="bgtb" colspan="2">LPAG<br>申込/送金</th>
                <th class="bgtb" colspan="2">LPSM<br>申込/送金</th>
                <th class="bgtb" colspan="2">LPKR<br>申込/送金</th>
              </tr>
            </thead>
            <tbody>
            @foreach($item2 as $k=>$hm)
              @if ($loop->last)
              <tr style="background-color:#ffe4c4;">
                <td>合計</td>
                <td><span style="color:red;">{{$hm->LP}}</span></td>
                <td>{{$hm->LP振込}}</td>
                <td><span style="color:red;">{{$hm->LPJO}}</span></td>
                <td>{{$hm->LPJO振込}}</td>
                <td><span style="color:red;">{{$hm->LPMT}}</span></td>
                <td>{{$hm->LPMT振込}}</td>
                <td><span style="color:red;">{{$hm->LPAG}}</span></td>
                <td>{{$hm->LPAG振込}}</td>
                <td><span style="color:red;">{{$hm->LPSM}}</span></td>
                <td>{{$hm->LPSM振込}}</td>
                <td><span style="color:red;">{{$hm->LPKR}}</span></td>
                <td>{{$hm->LPKR振込}}</td>
              </tr>
              @else
              <tr>
